...
class OrderBook(object):
    ...
    def execute(self, trader_buy, trader_sell, price, size, buy_sell, placed_time, execute_time, transactions):
        """
        Execution callback
        ...
        """
        print ("EXECUTE: %s BUY %s SELL %s %s @ %d EXCUTED_DIRECTIOND: %s PLACED: %s EXECUTED: %s" % (trader_buy, trader_sell, size, self.name, price,buy_sell,placed_time, execute_time))
    ...
    def limit_order(self, side, size, price, trader,last_price, current_time, transactions):
        """
        ...
        if side == 0: # buy order
            ...
            while price >= self.ask_min:
                ...
                while entries:
                    ...
                    buy_sell='BUY'
                    if entry.size < size:
                        last_price, transactions =self.cb(trader, entry.trader, price, entry.size, buy_sell, entry.placed_time, current_time, transactions)
                        size -= entry.size
                        entries.popleft()
                    else:
                        last_price, transactions =self.cb(trader, entry.trader, price, size, buy_sell, entry.placed_time, current_time, transactions)
                        if entry.size > size:
                            entry.size -= size
                        else:
                            entries.popleft()
                        self.order_id += 1
                        return self.order_id, last_price, transactions
            ...
            while price <= self.bid_max:
                ...
                while entries:
                    ...
                    buy_sell='SELL'
                    if entry.size < size:
                        last_price, transactions =self.cb(entry.trader, trader, price, entry.size, buy_sell, entry.placed_time, current_time, transactions)
                        size -= entry.size
                        entries.popleft()
                    else:
                        last_price, transactions =self.cb(entry.trader, trader, price, size, buy_sell, entry.placed_time, current_time, transactions)
                        if entry.size > size:
                            entry.size -= size
                        else:
                            # entry.size == size
    ...
    def _render_order(self, level, maxlen=40):
        """
        Renders a single price point level of the order book.
        """
