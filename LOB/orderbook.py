...
class OrderBook(object):
    ...
    def limit_order(self, side, size, price, trader,last_price):
        """
        ...
        if side == 0: # buy order
            ...
            while price >= self.ask_min:
                ...
                while entries:
                    entry = entries[0]
                    if entry.size < size:
                        last_price=self.cb(trader, entry.trader, price, entry.size)
                        size -= entry.size
                        entries.popleft()
                    else:
                        last_price=self.cb(trader, entry.trader, price, size)
                        if entry.size > size:
                            entry.size -= size
                        else:
                            entries.popleft()
                        self.order_id += 1
                        return self.order_id, last_price
            ...
            while price <= self.bid_max:
                ...
                while entries:
                    entry = entries[0]
                    if entry.size < size:
                        last_price=self.cb(entry.trader, trader, price, entry.size)
                        size -= entry.size
                        entries.popleft()
                    else:
                        last_price=self.cb(entry.trader, trader, price, size)
                        if entry.size > size:
                            entry.size -= size
                        else:
                            # entry.size == size
    ...
    def _render_order(self, level, maxlen=40):
        ...
        Renders a single price point level of the order book.
        """
        ret = [[order.price, order.size, order.trader, order.order_id] for order in level]
        return ret
